	...
	def __init__(self, master, a):
		...
		for i,j in a.items():
			...
			if ("type" in j and j["type"] == "treeview"):
				...
				self.vars[i]["obj"].grid(column=1,row=h,pady=5,padx=5, sticky="w")
			h+= 1
		tkinter.Button(self.win, text="Add", command=lambda: self.add("add")).grid(column=1,row=h)
